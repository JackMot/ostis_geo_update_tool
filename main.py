...
def get_places_yandex(URL, KEY, PLACES_EN, CITIES_EN, LANG):
    for city in CITIES_EN:
        for PLACE in PLACES_EN:
            ...
            print(NAME)
            print(data)
            with open(NAME + '.en.json', 'w') as outfile:
...
def generate_scs_instances(json_r, CITY):
    ...
    for item in json.load(json_r)["features"]:
        ...
        path = "scs/" + str(CITY) + '/' + str(point['category'])
        if not os.path.exists(path):
            os.makedirs(path)
            print("Create dir :", path)
        environment = Environment(loader=FileSystemLoader("templates/"))
        template = environment.get_template("scs_sample.txt")
        filename = str(point['category']) + '_' + point['id'] + ".scs"
        ...
        with open(path + '/' + filename, mode="w", encoding="utf-8") as scs:
            scs.write(content)
        print("Create file : ", path + '/' + filename)
        item_counter += 1
    return item_counter
...
for city in cities_en:
    for place in places_en:
        name = str(city + '.' + place)
        with open("json.row/" + name + ".json", 'r') as json_row:
